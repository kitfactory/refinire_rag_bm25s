...
本プロジェクトはrefinire-ragにBM25sのデータベースをVectorStoreクラスのサブクラスとして提供する。プラグインプロジェクトです。

BM25sは本来VectorSearchではありませんが、使用方法として同等ですので、VectorSearchのサブクラスとして提供します。

refinire-ragとbm25s-jに依存します。
